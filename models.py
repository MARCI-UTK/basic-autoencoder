...
class HFImageEncoder(nn.Module):
    def __init__(self, model_name, from_scratch=True, pretrained=False, is_trainable=True):
        super().__init__()
...
class ImageDecoder(nn.Module):
    def __init__(self, emb_dim, num_channels=3):
        super().__init__()

        # Store parameters
        self.emb_dim = emb_dim
        self.shape_before_flattening = [32, 14, 14]
        ...
        self.block = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(8, num_channels, kernel_size=3, stride=1, padding=1),
            nn.Sigmoid()
        )
